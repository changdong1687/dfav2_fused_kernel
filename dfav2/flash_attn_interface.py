...
if USE_TRITON_ROCM:
    ...
    import flash_attn_2_cuda_ours as flash_attn_gpu
...
def _flash_attn_forward(
    ...
    return_softmax: bool,
    window_size_lefts: torch.Tensor,
    window_size_rights: torch.Tensor,
    need_residual:torch.Tensor,
    seqlen_q_vision: int,
    seqlen_k_vision: int,
    is_hw_residual: bool = False,
    is_headwise_arrow: bool=False,
    is_headwise_half_arrow: bool=False,
    is_headwise_win: bool=False,
    is_win_attn: bool=False,
    is_headwise_arrow_trans: bool = False,

) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    # print(f"OURS: call _flash_attn_forward in flash_attn_interface.py...")
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    out, output_residual, softmax_lse, S_dmask, rng_state = flash_attn_gpu.fwd(
        q,
        k,
        v,
        None,
    ...
    return out, output_residual, softmax_lse, S_dmask, rng_state
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        out_padded, out_residual, softmax_lse, S_dmask, rng_state = _wrapped_flash_attn_forward(
            ...
            alibi_slopes=alibi_slopes,
            return_softmax=return_softmax and dropout_p > 0,
            window_size_lefts=window_size_lefts,
            window_size_rights=window_size_rights,
            seqlen_q_vision= -1,
            seqlen_k_vision=-1,
            is_win_attn=True,
        )
...
class FlashAttnFuncHeadWiseWithResidual(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        if need_residual is None:
            need_residual=torch.zeros(q.shape[2], 1, dtype=torch.int32, device=q.device)

            # window_size_rights[0] = q.shape[1] * 2

        # print("HW: call FlashAttnFuncHeadWin in flash_attn_interface.py")
        out_padded, out_residual, softmax_lse, S_dmask, rng_state = _wrapped_flash_attn_forward(
            q,
            k,
            v,
            ...
            window_size_rights=window_size_rights,
            need_residual=need_residual,
            seqlen_q_vision = seqlen_q_vision,
            seqlen_k_vision = seqlen_k_vision,
            is_hw_residual = is_hw_residual,
            is_headwise_arrow = is_headwise_arrow,
            is_headwise_half_arrow = is_headwise_half_arrow,
            is_headwise_win = is_headwise_win,
            is_win_attn = is_win_attn,
            is_headwise_arrow_trans = is_headwise_arrow_trans,
        ...
        if is_grad:
            ...
            ctx.deterministic = deterministic
        out = out_padded[..., :head_size_og]
        return out, out_residual if not return_softmax else (out, softmax_lse, S_dmask)
    

class FlashAttnFuncHeadWise(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
...
def headwise_arrow_attn_with_residual(
    q,
    k,
    v,
    dropout_p=0.0,
    ...
    # print("\033[35mFullArrow\033[0m: call headwise_arrow_attn in flash_attn_interface.py")
    return FlashAttnFuncHeadWiseWithResidual.apply(
        q,
        k,
        v,
        dropout_p,
...
def headwise_arrow_attn_trans(
    ...
    # print("\033[35mFullArrowTrans\033[0m: call trans_headwise_arrow_attn in flash_attn_interface.py")
    return FlashAttnFuncHeadWise.apply(
        q,
        k,
        v,
        dropout_p,
...
def headwise_half_arrow_attn(
    ...
    # print("\033[35mHalfArrow\033[0m: call headwise_half_arrow_attn in flash_attn_interface.py")
    return FlashAttnFuncHeadWise.apply(
        q,
        k,
        v,
...
def headwise_window_attn(
    ...
    # print("\033[35mHeadWindow\033[0m: call headwise_window_func in flash_attn_interface.py")
    return FlashAttnFuncHeadWise.apply(
        q,
        k,
        v,
...
def flash_attn_func(
    ...
    Return:
        ...
        S_dmask [optional, if return_attn_probs=True]: (batch_size, nheads, seqlen, seqlen).
            ...
            pattern (negative means that location was dropped, nonnegative means it was kept).
    """
    # print("OURS: call flash_attn_func in flash_attn_interface.py")
    # print("\033[35mWindowAttn\033[0m: call flash_attn_func in flash_attn_interface.py")
