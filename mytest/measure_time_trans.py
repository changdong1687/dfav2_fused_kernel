...
def main(argv):
    ...
    if args.full_arrow:
        from flash_attn_ours import  headwise_arrow_attn_trans
        hw_fa_times = []
        flex_full_arrow_times = []
        ...
        for _ in range(100):
            headwise_arrow_attn_trans(q, k, v, window_sizes=window_sizes, seqlen_q_vision = seqlen_vision, seqlen_k_vision = seqlen_vision)
        torch.cuda.synchronize()
        ...
        for _ in range(100): 
            ...
            o_hw_full_arrow = headwise_arrow_attn_trans(q, k, v, window_sizes=window_sizes, seqlen_q_vision = seqlen_vision, seqlen_k_vision = seqlen_vision)
        ...
        print(f"ours full arrow trans time: {hw_full_arrow_mean}")
        if args.flex_full:
        ...
        if args.ori:
            ...
            print(f"speedup ours trans: {ori_full_mean/hw_full_arrow_mean:.4f}")
