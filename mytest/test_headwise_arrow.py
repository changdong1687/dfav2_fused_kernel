...
def main(argv):
    ...
    o_flex_full_arrow = flex_headwin_attn(q.permute(0, 2, 1, 3).contiguous(), 
                               k.permute(0, 2, 1, 3).contiguous(), 
                               v.permute(0, 2, 1, 3).contiguous(), 
                               args.seqlen_vision, args.seqlen_text, flex_masks_full_arrow, (128, 128)).permute(0, 2, 1, 3).contiguous()
    
    o_sdpa_full_arrow = sdpa_attn(q.permute(0, 2, 1, 3).contiguous(),
                            k.permute(0, 2, 1, 3).contiguous(), 
                            v.permute(0, 2, 1, 3).contiguous(), 
                            mask=window_masks_full_arrow,
                            ).permute(0, 2, 1, 3)


    o_headwise_half_arrow = headwise_half_arrow_attn(q, k, v, window_sizes=window_sizes, seqlen_k_vision=args.seqlen_vision, seqlen_q_vision=args.seqlen_vision)

    wh, wtr, wtc = window_masks_half_arrow.shape

    flex_masks_half_arrow = torch.zeros((wh, (wtr + block_sizes[0] - 1) // block_sizes[0] * block_sizes[0], (wtc + block_sizes[1] - 1) // block_sizes[1] * block_sizes[1]), device=window_masks_half_arrow.device, dtype=window_masks_half_arrow.dtype)
    flex_masks_half_arrow[:, :wtr, :wtc] = window_masks_half_arrow
    
    o_flex_half_arrow = flex_headwin_attn(q.permute(0, 2, 1, 3).contiguous(), 
                               k.permute(0, 2, 1, 3).contiguous(), 
                               v.permute(0, 2, 1, 3).contiguous(), 
                               args.seqlen_vision, args.seqlen_text, flex_masks_half_arrow, (128, 128)).permute(0, 2, 1, 3).contiguous()
    
    o_sdpa_half_arrow = sdpa_attn(q.permute(0, 2, 1, 3).contiguous(),
                            k.permute(0, 2, 1, 3).contiguous(), 
                            v.permute(0, 2, 1, 3).contiguous(), 
                            mask=window_masks_half_arrow,
                            ).permute(0, 2, 1, 3)
    ...
    print(f"{'-'*64}\n{'full arrow attn':^21}\n{'-'*64}")
    print("{:<25}=={:>25}: \033[92m{!s:6>}\033[0m".format("o_flex_full_arrow", "o_headwise_full_arrow", torch.allclose(o_flex_full_arrow, o_headwise_full_arrow, 1e-3, 1e-3)))
    print("{:<25}=={:>25}: \033[92m{!s:6>}\033[0m".format("o_sdpa_full_arrow", "o_headwise_full_arrow", torch.allclose(o_sdpa_full_arrow, o_headwise_full_arrow, 1e-3, 1e-3)))
    print("{:<25}=={:>25}: \033[92m{!s:6>}\033[0m".format("o_flex_full_arrow", "o_sdpa_full_arrow", torch.allclose(o_flex_full_arrow, o_sdpa_full_arrow, 1e-3, 1e-3)))
    print(f"{'-'*64}\n{'half arrow attn':^21}\n{'-'*64}")
    print("{:<25}=={:>25}: \033[92m{!s:6>}\033[0m".format("o_flex_half_arrow", "o_headwise_half_arrow", torch.allclose(o_flex_half_arrow, o_headwise_half_arrow, 1e-3, 1e-3)))
    print("{:<25}=={:>25}: \033[92m{!s:6>}\033[0m".format("o_sdpa_half_arrow", "o_headwise_half_arrow", torch.allclose(o_sdpa_half_arrow, o_headwise_half_arrow, 1e-3, 1e-3)))
    print("{:<25}=={:>25}: \033[92m{!s:6>}\033[0m".format("o_flex_half_arrow", "o_sdpa_half_arrow", torch.allclose(o_flex_half_arrow, o_sdpa_half_arrow, 1e-3, 1e-3)))
