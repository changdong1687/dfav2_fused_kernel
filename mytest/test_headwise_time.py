...
######################################################
#       test headwise window attention time
######################################################
    hw_win_ours_times, hw_win_mean = [], []
    flex_hw_times = []
    output_hw_rights = []
    flex_hw_mean = []
    ...
    for i in range(len(win_size_half)):
        ...
        for _ in range(100):
            headwise_window_attn(q, k, v, window_sizes=window_sizes, seqlen_q_vision = args.seqlen_vision, seqlen_k_vision = args.seqlen_vision)
        torch.cuda.synchronize()

        start_ours_hw_win = torch.cuda.Event(True)
        end_ours_hw_win =  torch.cuda.Event(True)
        
        
        for _ in range(100): 
            start_ours_hw_win.record()
            hw_win_ours = headwise_window_attn(q, k, v, window_sizes=window_sizes, seqlen_q_vision = args.seqlen_vision, seqlen_k_vision = args.seqlen_vision)
            end_ours_hw_win.record()
            torch.cuda.synchronize()
            tot_hw_win_ours = (start_ours_hw_win.elapsed_time(end_ours_hw_win)) 
            hw_win_ours_times.append(tot_hw_win_ours)
        hw_win_mean.append(np.mean(np.array(hw_win_ours_times)))

        window_masks_hw, _ = create_headwise_window_mask(H, args.seqlen_vision+args.seqlen_text, window_sizes, (128, 128))
        flex_masks_hw = torch.zeros(window_masks_hw.shape[0], 
                                (N + block_size[0] - 1) // block_size[0] * block_size[0], 
                                (N + block_size[1] - 1) // block_size[1] * block_size[1],
                                dtype=window_masks_hw.dtype,
                                device=window_masks_hw.device)
        flex_masks_hw[:, :N, :N] = window_masks_hw
        ...
        for _ in range(100):
            flex_headwin_attn(qp,
                                    kp,
                                    vp, 
                                    args.seqlen_vision+args.seqlen_text, 0, flex_masks_hw, (128, 128))
            torch.cuda.synchronize()
        ...
        # print("warm up done!")
        start_flex_hw = torch.cuda.Event(True)
        end_flex_hw = torch.cuda.Event(True)
        for _ in range(100): 
            start_flex_hw.record()
            o_flex_hw = flex_headwin_attn(qp,
                                    kp,
                                    vp,
                                    args.seqlen_vision+args.seqlen_text, 0, flex_masks_hw, (128, 128))
            # o_flex = flex_headwin_attn(q.permute(0, 2, 1, 3).contiguous(), 
            ...
            end_flex_hw.record()
            torch.cuda.synchronize()
            tot_flex_hw = (start_flex_hw.elapsed_time(end_flex_hw)) 
            flex_hw_times.append(tot_flex_hw)
        flex_hw_mean.append(np.mean(np.array(flex_hw_times)))
        output_hw_rights.append(torch.allclose(hw_win_ours, o_flex_hw.permute(0, 2, 1, 3), 1e-3, 1e-3))
    ...
    for i in range(len(win_size_half)):
        ...
        print(f"ours_full_arrow == felx_hw: \033[92m{output_hw_rights[i]}\033[0m")
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("ous time:", hw_win_mean[i]))
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("flex time:", flex_hw_mean[i]))
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("original full attn time:", ori_full_mean))
    

######################################################
#       test headwise full arrow attention time
######################################################
    hw_full_arrow_times, hw_full_arrow_mean = [], []
    ...
    for i in range(len(win_size_half)):
        ...
        for _ in range(100): 
            ...
            torch.cuda.synchronize()
            tot_hw_win_ours = (start_hw_full_arrow.elapsed_time(end_hw_full_arrow)) 
            hw_full_arrow_times.append(tot_hw_win_ours)
        hw_full_arrow_mean.append(np.mean(np.array(hw_full_arrow_times)))
    ...
    for i in range(len(win_size_half)):
        print(f"{'-'*64}\n{f'window-size: \033[96m({win_size_half[i]}, {win_size_half[i]})\033[0m local attn':^64}\n{'-'*64}")
        print(f"{'-'*64}\n{f'{args.full_num}/{args.num_head} heads do full {args.num_head - args.full_num} heads do {win_size_half[i]*2} local attn':^64}\n{'-'*64}\n")
        print(f"ours_full_arrow == felx_full_arrow: \033[92m{output_hw_full_arrows_rights[i]}\033[0m")
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("ous time:", hw_full_arrow_mean[i]))
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("flex time:", flex_full_arrow_mean[i]))
...
######################################################
    ...
    for i in range(len(win_size_half)):
        window_sizes = torch.zeros((H, 2), device=device, dtype=torch.int32) - 1
        window_sizes[:, :] = win_size_half[i]   
        window_sizes[:args.half_num, :] = -1
        # torch.cuda.empty_cache()
        for _ in range(100):
            headwise_arrow_attn(q, k, v, window_sizes=window_sizes, seqlen_q_vision = args.seqlen_vision, seqlen_k_vision = args.seqlen_vision)
        torch.cuda.synchronize()

        start_hw_half_arrow = torch.cuda.Event(True)
        end_hw_half_arrow =  torch.cuda.Event(True)
        
        
        for _ in range(100): 
            start_hw_half_arrow.record()
            o_hw_half_arrow = headwise_arrow_attn(q, k, v, window_sizes=window_sizes, seqlen_q_vision = args.seqlen_vision, seqlen_k_vision = args.seqlen_vision)
            end_hw_half_arrow.record()
            torch.cuda.synchronize()
            tot_hw_win_ours = (start_hw_half_arrow.elapsed_time(end_hw_half_arrow)) 
            hw_half_arrow_times.append(tot_hw_win_ours)
    ...
    for i in range(len(win_size_half)):
        ...
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("ous time:", hw_half_arrow_mean[i]))
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("flex time:", flex_half_arrow_mean[i]))
        print("\t{:<24}\033[35m{:>12.6f}\033[0m".format("original time:", ori_full_mean))
