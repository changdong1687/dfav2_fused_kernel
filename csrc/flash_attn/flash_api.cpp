...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      const int seqlen_q_vision,
                      const int seqlen_k_vision,
                      // device pointers
                      const at::Tensor q,
                      const at::Tensor k,
                      const at::Tensor v,
                      at::Tensor out,
                      at::Tensor out_residual,
                      void *cu_seqlens_q_d,
                      void *cu_seqlens_k_d,
                      void *seqused_k,
                      void *p_d,
                      ...
                      const float softcap,
                      const bool is_hw_residual,
                      const bool is_headwise_arrow,
                      const bool is_headwise_half_arrow,
                      const bool is_headwise_win,
                      const bool is_win_attn,
                      const bool is_headwise_arrow_trans,
                      const at::Tensor window_size_lefts=at::Tensor(),
                      const at::Tensor window_size_rights= at::Tensor(),
                      const at::Tensor need_residual=at::Tensor(),
    ...
    params.o_res_ptr = out_residual.data_ptr();
    params.o_res_row_stride = out_residual.stride(-3);
    params.o_res_head_stride = out_residual.stride(-2);
    ...
    if (cu_seqlens_q_d == nullptr) {
        ...
        params.o_res_batch_stride = out_residual.stride(0);
        if (seqlenq_ngroups_swapped) {
             params.q_batch_stride *= seqlen_q;
             params.o_batch_stride *= seqlen_q;
             params.o_res_batch_stride *= seqlen_q;
        }
    }
    ...
    params.seqlen_q_vision = seqlen_q_vision;
    params.seqlen_k_vision = seqlen_k_vision;
    params.seqlen_q_text = seqlen_q - seqlen_q_vision;
    params.seqlen_k_text = seqlen_k - seqlen_k_vision;

    params.is_hw_residual = is_hw_residual; 
    params.is_headwise_arrow = is_headwise_arrow;
    params.is_headwise_half_arrow = is_headwise_half_arrow;
    params.is_headwise_win = is_headwise_win;
    params.is_win_attn = is_win_attn;
    params.is_headwise_arrow_trans = is_headwise_arrow_trans;
    ...
    // for (int i = 0; i < h; i++){
    //     if (window_size_lefts[i].item<int32_t>() < 0 && window_size_rights[i].item<int32_t>() >= 0 ) { printf("OURS: in set_param window_size_lefts[%d] %d\n", i, window_size_lefts[i].item<int32_t>()); window_size_lefts[i] = seqlen_k; }
    //     if ( window_size_lefts[i].item<int32_t>() >= 0 && window_size_rights[i].item<int32_t>() < 0 ) { printf("OURS: in set_param window_size_rights[%d] %d\n", i, window_size_rights[i].item<int32_t>()); window_size_rights[i] = seqlen_k; }
    // }

    params.need_residual_ptr = static_cast<int32_t *>(need_residual.data_ptr());
    params.window_size_lefts_ptr = static_cast<int32_t *>(window_size_lefts.data_ptr());
    params.window_size_rights_ptr = static_cast<int32_t *>(window_size_rights.data_ptr());
    if (is_win_attn){
        params.switch_local = params.window_size_left >= 0 || params.window_size_right >= 0;
    }
    else{
        params.switch_local = (torch::any(torch::ge(window_size_lefts, 0)).item<bool>() || torch::any(torch::ge(window_size_rights, 0)).item<bool>()); // && (seqlen_k_vision > 0 && seqlen_q_vision > 0);
    }
...
void run_mha_fwd(Flash_fwd_params &params, cudaStream_t stream, bool force_split_kernel=false) {
    // printf("call run_mha_fwd in %s\n", __FILE__);
    FP16_SWITCH(!params.is_bf16, [&] {
        HEADDIM_SWITCH(params.d, [&] {
            BOOL_SWITCH(params.is_causal, Is_causal, [&] {
                // printf("OURS: in flash_api.cpp call run_mha_fwd_ num_splits: %d\n", params.num_splits);
                if (params.num_splits <= 1 && !force_split_kernel) {  // If we don't set it num_splits == 0
                    // printf("OURS: in flash_api.cpp call run_mha_fwd_ fp16:%d kHeadDim: %d num_splits: %d\n", (!params.is_bf16), kHeadDim, params.num_splits);
...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x round_multiple(head_size, 8)
        ...
        at::Tensor &window_size_lefts,
        at::Tensor &window_size_rights,
        at::Tensor &need_residual,        // mark which head need to return residual
        const int seqlen_q_vision = -1,
        const int seqlen_k_vision = -1,
        const bool is_hw_residual = false,
        const bool is_headwise_arrow = false,
        const bool is_headwise_half_arrow = false,
        const bool is_headwise_win = false,
        const bool is_win_attn = false,
        const bool is_headwise_arrow_trans = false) {
    ...
    //     if (window_size_lefts[i].item<int32_t>() >= seqlen_k) { printf("OURS: window_size_lefts[%d]: %d\n", i, window_size_lefts[i].item<int32_t>()); window_size_lefts[i] = -1; }
    //     if (window_size_rights[i].item<int32_t>() >= seqlen_k) { printf("OURS: window_size_rights[%d]: %d\n", i, window_size_rights[i].item<int32_t>()); window_size_rights[i] = -1; }
        if (is_causal) { window_size_rights[i] = 0; }
    }
    ...
    const int seqlenq_ngroups_swapped = seqlen_q == 1 && num_heads > num_heads_k && torch::any(torch::lt(window_size_lefts, 0)).item<bool>() && torch::any(torch::lt(window_size_rights, 0)).item<bool>() && p_dropout == 0.f && head_size % 8 == 0 && !alibi_slopes_.has_value();
    ...
    // printf("call in mha_fwd in flash_api.cpp before out_residual init!\n");
    at::Tensor out_residual = torch::empty_like(q);             // !! here we add residual output
    CHECK_DEVICE(out_residual);
    out_residual.zero_();
    ...
    set_params_fprop(params,
                     ...
                     is_hw_residual,
                     is_headwise_arrow,
                     is_headwise_half_arrow,
                     is_headwise_win,
                     is_win_attn,
                     is_headwise_arrow_trans,
                     window_size_lefts,
                     window_size_rights,
                     need_residual
                     );
    ...
    // printf("call in mha_fwd in flash_api.cpp\n");
    return {out, out_residual, softmax_lse, p, rng_state};
}
